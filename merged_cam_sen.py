...
def calculate_accel_magnitude(x, y, z):
    x *= SCALE_FACTOR_ADXL_8G
    y *= SCALE_FACTOR_ADXL_8G
    z *= SCALE_FACTOR_ADXL_8G
...
def main():
    ...
    try:
        while True:
            ...
            if stream:
                try:
                    ...
                    stream = None
                    print("Stream-Verbindung verloren.")
            else:
                # Alle 3 Sekunden erneut versuchen
                if time.time() - last_stream_retry > 3:
                    stream = open_stream()
                    ...
                    for (ex, ey, ew, eh) in eyes:
                        ...
                        if contours:
                            ...
                            if area_new > 10:
                                ...
                                dt = time.time() - previous_time if previous_time else 0
                                if previous_pos and dt > 0:
                                ...
                                area = area_new
                                delta = area - previous_area if previous_area else 0
                                previous_area = area
            ...
            # === Erregungswert berechnen & CSV schreiben ===
            erregungswert = calculate_erregungswert(mic_trigger, speed, area, delta,
            ...
            if addr:
                try:
                    sock.sendto(f"Erregung:{erregungswert}".encode(), addr)
                except Exception as e:
                    print(f"\nUDP-Sendeproblem: {e}")
        ...
        print("Socket und Fenster geschlossen.")
